 ...
 */
export const inject = ["webServer", "storageDomain", "settings", "credentials"];

/**
 * Host plugin body. 注册素材路由（工单 02）、导入 API（工单 07）与 ai-title 路由
 * （工单 16-03）；路由通过 `ctx.effect` 托管，fiber 卸载时自动清理。导入 API 打开
 * KV domain（async），故 apply 为 async。
 *
