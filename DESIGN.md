...
- **会话气泡列（ADR-0007/0018）**：角色左侧竖排、自下而上生长；**归组模型**：一归组气泡 = 一顶层会话及其全部 subagent 后代（沿 `parentId` 上溯至根祖先，孤儿自成顶层）；气泡 = 标题 + 状态点（运行中金呼吸点 / 已完成石绿实心点 / 等待用户交互朱砂涟漪点）+ **子代理徽标**（`▸N`/`▾N` 计后代总数，有运行中后代时前缀金呼吸迷你点）；点击气泡本体经 `sessions.open(id)` 跳转对应会话（`pointer-events:auto` + `cursor:pointer`，反转台词气泡穿透原则，仅限气泡本体与徽标）；点击徽标原地展开子气泡列表（向左缩进 12px + 弱化背景 + 左竖连接线，再点收起）；当前会话气泡 `--jx-gold` 金描边高亮，current 在后代中时描边传播至根祖先且该组强制展开；数量上限默认 10（1-10 可配置，SettingsCard「角色」section）只约束顶层归组气泡，溢出折叠为「+N」点击原地展开；**等待用户交互特效（ADR-0020）**：`pendingInteraction` 非空的气泡描边转 `--jx-seal` 朱砂、点位换涟漪扩散环、叠加时播放一次印章按压强调（350ms），朱砂描边优先级高于当前/hover 金描边，且豁免折叠、永驻可见（moreCount 不计；分组模型下按组聚合——组内任一成员等待交互则整组豁免、根气泡朱砂描边），aria-label 追加「等待确认」，reduced-motion 停动效保留静态朱砂点/描边；出现 150ms 淡入 / 消失 100ms 淡出，`prefers-reduced-motion` 全关；随盒整体移动。
...
- **保留模式投放区与拖拽手势（ADR-0022）**：设置卡「查看后保留气泡」总开关①开时，已完成类气泡可按住拖拽（>8px 进拖态、DOM transform 跟手；running/等待交互气泡呈 `.dragForbidden` 禁止态且不进臂态——禁拖不禁点，8px 内松手仍是原生点击）；两个投放区由 `data-jx-zone` 标记供落点解析（elementFromPoint → closest）：**收起区**（近放，整列正下方 8px、虚线素线弱化语言）= 本地 dismissed 记账暂时隐藏提醒，可逆，completed 上升沿自动清账防旧收起吞新完成，Delete/Backspace 键盘等效收起；**归档区**（远放·角色脚边，浮层盒正下方居中让位状态文案、朱砂警示描边 + title 提示「归档后从列表隐藏，不可恢复」，受第二开关「拖拽归档会话」②主从灰显控制）= 调 `workspaces.archiveSession` 真归档，侧边栏同步隐藏、不可逆，失败静默（气泡不消失即失败信号），当前正停留会话拖入呈禁止态不发起调用；归档排除集每次从宿主 `archivedSessionIds` 快照派生（归档权威在 SDK，本地不重复记账，已归档永不复活）。拖拽发生过必吞紧随的合成 click（setPointerCapture 后浏览器仍会在捕获元素上合成 click——suppressClickRef + 容器捕获阶段消费一次）。投放区静态无动画，reduced-motion 天然免分支；随盒整体移动。
- **DOM 健康守卫（overlay-img-guard）**：浮层盒直接子级 `<img>` 集合恒等于 React 挂载 ref 集合（主图 + 焦点切换瞬态 underlay，≤2）；`MutationObserver` 即时裁掉任何外部来源塞入的越界 img，裁剪事件节流 `console.warn` 告警（健康环境永不触发，触发即存在外部写入方的诊断信号）。
