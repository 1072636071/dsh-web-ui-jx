...
**用户视角交付：** 流式输出时壁纸打标不再阻塞主线程；毛玻璃只作用于必要区域、滚动与重绘更快；warp 特效行为与文档一致（真正「停下淡出」或明确「无淡出」二选一）；点击不再触发强制同步布局；飘落特效走纯合成层（若实测成立）。
...
- [ ] `welcome-backdrop.test.ts` rAF 批处理/前置过滤扩展用例全绿
- [ ] H1/M2 暗/亮双主题视觉回归通过（截图对比）
- [ ] 19-03 二选一落地且 `warp-controller.test.ts` 对齐语义
- [ ] `warp.ts` 无 `void el.offsetWidth` 强制同步布局
