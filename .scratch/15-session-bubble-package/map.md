...
- 工单 06 已完成：发布清单修正 + pack/publish dry-run + 包名可用性确认，6 张工单全部 closed
- 待执行操作（需用户）：①npm login 后 publish 库与薄壳；②真实 DSH 宿主验证薄壳注入渲染与 `dsh plugin add link:` 装完即用
- 全部 6 张工单状态：01–06 resolved（03 项待外部环境验证：薄壳渲染/安装、两包实际 publish）
