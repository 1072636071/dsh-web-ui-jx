...
- 不做「打开会话并滚动定位到某条问话」——官方 API 无此能力，作为已知限制记录（ADR-0028 附注）；seq 随数据下发留待官方开放后接线。**2026-08-28 深挖调研（工单 004）钉死结论**：官方 anchor 是「视口稳定」机制而非定位功能，且官方产品自身搜索点击同样只跳会话（源码注释明示不定位会话内事件）；插件四条通道（open 无参 / 锚 key 不可构造 / chatScroll 闭包私有 / viewRequest 不被 ChatView 消费）逐条封死；曾评估的 `turnTail` 零改动暗道因**与官方滚动补偿竞争**、精度仅到 Turn 尾部而**明确不采用**（用户拍板）。详见同目录 `research-official-locate.md`（证据全文：`docs/memorial/014-bubble-hover-preview-last-message/sub-task/004-official-locate-internals.md`）。
- 不改官方宿主/前端源码。
- 不改既有气泡点击/保留/收起交互契约。
