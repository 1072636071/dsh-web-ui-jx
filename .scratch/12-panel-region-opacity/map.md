...
## 工单
- **issues/01-region-alpha-model.md**（ready-for-agent，无阻塞）：区域 alpha 配置 + CSS token + remap 修正 + 运行时写/移（含测试）。
- **issues/02-region-slider-ui.md**（ready-for-agent，Blocked by 01）：设置卡「其余面板」+ 五根区域滑杆 UI 接线。
- **issues/03-acceptance-verify.md**（ready-for-agent，Blocked by 02）：build / verify / 全量测试绿 + 实机走查。
