# ADR-0031 — 气泡 hover 内容预览：数据源取宿主服务端 `sessionController.inspect`

## 状态

已接受（2026-08-28 实施，PRD 14 工单 01–04）。
...
- 从 events 顺序收集全部（时序正序输出，末条恒为最新——「默认展开最后一个胶囊」恒成立的数据保证） `type==='user/message'` 且 `data.source.kind==='user'`（直接用户消息，排除 plugin/notice/recall 合成）的事件，提取 `content` 中 `type==='text'` 的 `text` 拼接，作为「问话胶囊」数据（每条含文本 + 事件 seq，seq 为将来定位留位）；
