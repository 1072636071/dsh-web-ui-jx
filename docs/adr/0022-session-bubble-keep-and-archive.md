...
- **归档排除的豁免边界（对 PRD 用户故事14「已归档彻底不再出现」的限定）**：
  running 气泡与等待交互（pending）气泡的紧急可见性压过归档排除集——
  即使某会话已被归档，只要它仍在运行或等待用户首肯，气泡列仍显示该行
  （紧急信号优先，与折叠豁免同源语义）。这是测试钉死的有意行为
  （「archived > dismissed > kept 优先级网格；running/pending 豁免压过
  归档集」用例），非实现偏差：绝对隐匿只对已完成且无等待交互的条目成立。
