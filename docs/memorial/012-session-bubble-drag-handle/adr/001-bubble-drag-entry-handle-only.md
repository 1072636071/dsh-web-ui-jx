...
> memorial 内部存档；已同步至项目全局 `docs/adr/0026-bubble-drag-entry-handle-only.md`。
> **状态更新**：本 ADR 的 D1（手柄唯一拖拽入口）已按用户反馈改型为「手柄点击直接收起」，
> 拖拽手势系统、投放区、归档区均已移除。保留为历史记录。
...
- **原始后果（已失效）**：整泡拖拽习惯失效、触屏命中热区缩小等——不再适用。
- **改型后后果**：
  - 交互极度简化：点击即收起，零学习成本；
