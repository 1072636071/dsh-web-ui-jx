...
**状态**：草稿（memorial 007 内部）
**日期**：2026-08-21
**关联**：ADR-0008（会话级状态机 + 焦点仲裁）、ADR-0009（表情体系）、ADR-0010（焦点层防抖 + 并行驻留 + 摸鱼彩蛋）、memorial 004 D12/D14（惊吓触发语义，本次落地）
...
- 新增 runtime 公共 API（如 `poke()`）与 `computeSnapshot` 分支；CharacterOverlay 增加点击检测与显式台词控制。
